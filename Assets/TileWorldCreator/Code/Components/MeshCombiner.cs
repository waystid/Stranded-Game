...
namespace GiantGrey.TileWorldCreator.Utilities
{
	...
	public class MeshCombiner : MonoBehaviour
	{
		...
		public bool resetVertexColor = true;
		...
		private void CombineMeshesWithSingleMaterial(bool showCreatedMeshInfo)
		{
			...
			// If there are no child meshes to combine, keep the existing mesh intact
			...
			// if (resetVertexColor)
			// {
			// 	for (var v = 0; v < meshFilters[0].mesh.colors.Length; v ++)
			// 	{
			// 		meshFilters[0].mesh.colors[v] = Color.black;
			// 	}
			// }
		...
		private void CombineMeshesWithMutliMaterial(bool showCreatedMeshInfo)
		{
			...
			UnityEngine.Rendering.ShadowCastingMode _shadowCastingMode = UnityEngine.Rendering.ShadowCastingMode.On;
			...
			for(int i = 0; i < meshFilters.Length-1; i++)
			{
				...
				{
					_shadowCastingMode = meshRenderers[i+1].shadowCastingMode;
					_receiveShadows = meshRenderers[i+1].receiveShadows;
			...
			// If nothing to combine (no materials/submeshes collected), do not overwrite existing mesh
			if (uniqueMaterialsList.Count == 0 || finalMeshCombineInstancesList.Count == 0 || verticesLength == 0)
			{
				return;
			}
			...
			meshRenderers[0].shadowCastingMode = _shadowCastingMode;
			meshRenderers[0].receiveShadows = _receiveShadows;
