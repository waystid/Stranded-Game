...
{
    public class BuildLayerItemDragManipulator : Manipulator
    {
        ...
        BuildLayerFolderListViewElement lastPickedFolder;

        public BuildLayerItemDragManipulator ( VisualElement _root, Configuration _asset, ConfigurationEditor _editor, string _layerGuid, string _folderGuid)
        {
            asset = _asset;
            editor = _editor;
            ...
            var _layerName = "";
            for (int j = 0; j < asset.buildLayerFolders.Count; j ++)
            {
                for (int k = 0; k < asset.buildLayerFolders[j].buildLayers.Count; k ++)
                {
                    if (asset.buildLayerFolders[j].buildLayers[k].guid == layerGuid)
                    {
                        _layerName = asset.buildLayerFolders[j].buildLayers[k].layerName;
                    }
                }
            }
        ...
        public void PointerUpHandler(MouseUpEvent _evt)
        {
            ...
            if (enabled)
            {
                ...
                {
                    for (int i = 0; i < asset.buildLayerFolders.Count; i ++)
                    {
                        if (asset.buildLayerFolders[i].guid == lastPickedFolder.name && asset.buildLayerFolders[i].guid != folderGuid)
                        {
                            for (int j = 0; j < asset.buildLayerFolders.Count; j ++)
                            {
                                for (int k = 0; k < asset.buildLayerFolders[j].buildLayers.Count; k ++)
                                {
                                    if (asset.buildLayerFolders[j].buildLayers[k].guid == layerGuid)
                                    {
                                        var _layer = asset.buildLayerFolders[j].buildLayers[k];
                                        asset.buildLayerFolders[i].buildLayers.Add(_layer);
                                        asset.buildLayerFolders[j].buildLayers.RemoveAt(k);
        ...
        public void PointerMoveHandler(MouseMoveEvent _evt)
        {
            ...
            var _pickedFolder = _pickElement.GetFirstAncestorOfType<BuildLayerFolderListViewElement>();
