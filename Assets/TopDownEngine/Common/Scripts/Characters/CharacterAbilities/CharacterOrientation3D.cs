...
namespace MoreMountains.TopDownEngine
{
	...
	public class CharacterOrientation3D : CharacterAbility
	{
		...
		protected const string _rotationSpeedAnimationParameterName = "YRotationSpeed";
		...
		protected int _rotationSpeedAnimationParameter;
		...
		protected virtual void RotateToFaceMovementDirection()
		{
			...
			{
				if (_controller.CurrentMovement.magnitude >= AbsoluteThresholdMovement)
				{
		...
		protected virtual void ComputeRelativeSpeeds()
		{
			...
			{
				_rotationSpeed -= Time.deltaTime * RotationSpeedResetSpeed;
			}
		...
		{
			RegisterAnimatorParameter(_rotationSpeedAnimationParameterName, AnimatorControllerParameterType.Float, out _rotationSpeedAnimationParameter);
		...
		{
			MMAnimatorExtensions.UpdateAnimatorFloat(_animator, _rotationSpeedAnimationParameter, _rotationSpeed, _character._animatorParameters, _character.RunAnimatorSanityChecks);
