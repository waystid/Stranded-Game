...
{	
	...
	public class WeaponAim3D : WeaponAim
	{
		...
		protected override void Initialization()
		{
			...
			_orientation3D = _weapon.Owner.FindAbility<CharacterOrientation3D>();
		}
		...
		protected virtual void ComputeLookRotation()
		{
			...
			CurrentAngle = ProcessAngleClamp(CurrentAngle);
			_lookRotation = Quaternion.Euler (CurrentAngle * Vector3.up);
		}

		protected virtual float ProcessAngleClamp(float angleToClamp)
		{
			if (!ClampAngle)
			{
				return angleToClamp;
			...
			angleToClamp = NormalizeAngle(angleToClamp);
			
			if (_orientation3D != null)
			{
				float modelRotation = NormalizeAngle(_orientation3D.ModelAngles.y);
				float angleDifference = Mathf.DeltaAngle(_orientation3D.ModelAngles.y, angleToClamp);

				if (angleDifference < MinimumAngle)
				{
					float newAngle = modelRotation + MinimumAngle;
					return newAngle;
				}
				if (angleDifference > MaximumAngle)
				{
					float newAngle = modelRotation + MaximumAngle;
					return newAngle;
				}
			}
			return angleToClamp;
